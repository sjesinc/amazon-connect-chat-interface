...
class ChatContainer extends Component {
  constructor(props) {
    ...
    this.submitChatAcceptedHandler = this.acceptChatSession.bind(this);
    EventBus.on("acceptChat", this.acceptChatSession.bind(this));
  ...
  componentWillUnmount() {
    ...
    EventBus.off(this.submitChatAcceptedHandler);
  }

  acceptChatSession(chatDetails, success, failure) {
    const logContent = {
    ...
    this.logger.info("Chat session meta data:", logContent);
    this.submitChatAcceptance(chatDetails, success, failure);
  }

  async submitChatAcceptance(input, success, failure) {
    console.log("Accept chat session for " + input.name);
    this.setState({ status: "Initiating" });

    try {
      console.log("Open agent chat session for " + input.name);
      const chatSession = await this.openAgentChatSession(input, input.name, input.region, input.stage);
      const attachmentsEnabled = input.featurePermissions[CHAT_FEATURE_TYPES.ATTACHMENTS];
  ...
  openAgentChatSession(chatDetails, name, region, stage) {
    console.log("Create agent chat session for " + chatDetails.name);
   ...
   * - ConnectionId
   *
   * Step 2: Connect to create chat session.
  ...
  async submitChatInitiation(input, success, failure) {
    console.log("Initiate and open customer chat session for " + input.name);
    this.setState({ status: "Initiating" });
