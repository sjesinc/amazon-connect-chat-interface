...
class ChatJSClient {
  ...
  constructor(chatDetails, region, stage) {
    console.log("New chat js client for " + chatDetails.name + " with contact " + chatDetails.contactId);
    ...
    if (chatDetails.participantRole === connect.ChatSession.SessionTypes.AGENT) {
      console.log("Creating " + chatDetails.participantRole + " chat js session for " + chatDetails.name);
      this.session = window.connect.ChatSession.create({
        chatDetails: chatDetails.agentMediaInfo, // REQUIRED
        options: { region: region },
        type: chatDetails.participantRole, // REQUIRED
        websocketManager: chatDetails.websocketManager, // REQUIRED
      });
    } else {
      this.session = connect.ChatSession.create({
        chatDetails: chatDetails.startChatResult,
        type: chatDetails.participantRole,  
        options: { region: region },
      });
    }
  }
...
class ChatSession {
  ...
  constructor(chatDetails, displayName, region, stage) {
    console.log("New chat session with contact " + chatDetails.contactId);
